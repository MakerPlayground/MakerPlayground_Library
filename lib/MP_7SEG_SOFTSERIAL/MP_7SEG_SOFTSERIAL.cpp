...
MP_7SEG_SOFTSERIAL::MP_7SEG_SOFTSERIAL(uint8_t tx, const String &tag)
	:serial(SendOnlySoftwareSerial(tx))
	,tag(tag)
{
}

void MP_7SEG_SOFTSERIAL::init()
{
	serial.begin(9600);
}

void MP_7SEG_SOFTSERIAL::showValue(double num)
{
	showFloat(value);
}

void MP_7SEG_SOFTSERIAL::showData(double num)
{
	showFloat(value);
}
